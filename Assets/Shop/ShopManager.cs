...
public class ShopManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Resources/Spores의 spore asset에 따라 상점 아이템 목록 자동 제작
        PopulateShop();
    }
    ...
    private void PopulateShop()
    {
        ...
        {
            GameObject shopItemInstance = Instantiate(shopItemPrefab, content);

            RectTransform rect = shopItemInstance.GetComponent<RectTransform>(); // 이 코드 없으면 상점 아이템 안 보임
            rect.localScale = Vector3.one;

            ShopItem shopItem = shopItemInstance.GetComponent<ShopItem>(); // 각 상점 아이템 정보를 spore 에셋과 연동
